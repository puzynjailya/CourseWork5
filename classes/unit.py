...
class BaseUnit(ABC):
    ...
    def _count_damage(self, target: BaseUnit) -> int:
        ...
        if target.stamina >= target.armor.stamina_per_turn:
            target_armor = target.armor.defence * target.armor
            damage = attack_damage - target_armor
            # self.stamina += (STAMINA_RECOVERY * self.unit_class.stamina) - self.weapon.stamina_per_hit
            # target.stamina += (STAMINA_RECOVERY * target.unit_class.stamina) - target.armor.stamina_per_turn
        else:
            damage = attack_damage
            # self.stamina = self.stamina - self.weapon.stamina_per_hit + (STAMINA_RECOVERY * self.unit_class.stamina)
            # target.stamina += STAMINA_RECOVERY * target.unit_class.stamina
...
class EnemyUnit(BaseUnit):
    ...
    def hit(self, target: BaseUnit) -> str:
        ...
        if not self._is_skill_used and random.randint(0,50) in randomlist:
            return self.unit_class.skill.use(user=self, target=target)
